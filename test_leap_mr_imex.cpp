/************************************************************************
 * File: test_leap_mr_imex.cpp                                              *
 * Programmers: Cory Mikida                                             *     
 *----------------------------------------------------------------------*
 * Testing Leap IMEX MRAM integration of chemistry with PyJac generated source
...
main()
{
  ...
  double corr_norm;
  std::shared_ptr<double> ic_slow(new double [4]);
  std::shared_ptr<double> ic_fast(new double [10]);
  ...
  ic_fast.get()[0] = 1001.0;            /* PyJacV2 convention: temperature goes at the beginning. */
  //y[1] = 101325.0;            /* PyJacV2 convention: pressure goes at the beginning. */
  ic_fast.get()[1] = 101426.325;            /* PyJacV2 convention: pressure goes at the beginning. */
  ic_fast.get()[2] = 0.0;               /* Leave out last species as well */
  ic_fast.get()[3] = 0.0;
  ic_fast.get()[4] = (0.1949817389200555/0.2072648773462248)/(2*15.9994);
  ic_fast.get()[5] = (0.01228313842616932/0.2072648773462248)/(2*1.00794);
  ic_fast.get()[6] = 0.0;
  ic_fast.get()[7] = 0.0;
  ic_fast.get()[8] = 0.0;
  ic_fast.get()[9] = 0.0;

  // Conserved variables for the fluid solve - rho, rho_u, rho_E.
  ic_slow.get()[0] = 0.2072648773462248;  /* density */
  ic_slow.get()[1] = 0.0;  /* mom_x */
  ic_slow.get()[2] = 0.0;  /* mom_y */
  ...
  *t_ptr = t_start;
  ...
  GasMixture = new Cantera::IdealGasMix("sanDiego.xml");
  for (int i=2; i < 10; i++) {
    massFractions[i-2] = ic_fast.get()[i]*mw[i-2];
  }
  massFractions[8] = 0.0;
  GasMixture->setState_TPY(ic_fast.get()[0], ic_fast.get()[1], massFractions);
  ...
  double tol = 1e-10;
  ic_slow.get()[3] = int_energy;  /* energy - with no fluid motion, this is just the internal energy */

  initialize(state, ic_fast, ic_slow, t_ptr, dt_ptr);
  ...
  for (iout=1, tout=T1; iout <= NOUT; iout++, tout += TADD) {
    ...
    for (int i=2; i < 10; i++) {
      massFractions[i-2] = state.ret_state_fast.get()[i]*mw[i-2];
    }
    massFractions[8] = 0.0;
    ...
    //       tout, state.ret_state_fast.get()[0]);
    printf("At t = %0.4e      y =%1.16e\n",
    ...
    for (int i=2; i < 10; i++) {
      ...
      outfile << std::setprecision(16) << state.ret_state_fast.get()[i] << std::endl;
    }
    std::ofstream outfile;
    outfile.open("Output/test_leap_temperature.txt", std::ios_base::app);
    outfile << std::setprecision(16) << state.ret_state_fast.get()[0] << std::endl;
  }
